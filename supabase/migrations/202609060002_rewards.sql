-- Execute once after the initial installation. Existing clients and visits are preserved.
begin;
create table if not exists ab_private.rewards(cut integer primary key,name text not null);
revoke all on ab_private.rewards from public,anon,authenticated;
insert into ab_private.rewards(cut,name) values
(5,'Bebidas gratis'),
(10,'Corte gratis'),
(15,'Facial gratis'),
(20,'Corte más bebida premium'),
(25,'Playera AB Premium'),
(28,'Agenda y pluma de AB Premier'),
(32,'Termo de AB Premier'),
(35,'Sobaquera de AB Premier y categoría Cliente Premium')
on conflict(cut) do update set name=excluded.name;
alter table public.courtesies add column if not exists prize_name text;
alter table public.clients add column if not exists is_demo boolean not null default false;
update public.courtesies c set prize_name=r.name from ab_private.rewards r where c.visit_number=r.cut and c.prize_name is null;
...
declare
 ...
 result jsonb; rows_json jsonb; rows2 jsonb; chosen_code text; before_json jsonb; uid uuid; prize text; next_cut integer;
