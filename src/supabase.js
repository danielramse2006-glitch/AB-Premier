...
export async function api(action,data=null,query=''){
 ...
 if(action==='export'){
  ...
  sheet.columns=[['Fecha','visited_at'],['Cliente','full_name'],['Código','code'],['Barbero','barber'],['Servicio','service'],['Puntos','points_earned'],['Visita','visit_number'],['Cortesía','courtesy_won']].map(([header,key])=>({header,key,width:23}));
